...
class ModuleBreadcrumb extends Module
{
	...
	protected function compile()
	{
		...
		if ($type == 'root')
		{
			if ($this->includeRoot)
			{
				$objFirstPage = $this->Database->prepare("SELECT id, alias FROM tl_page WHERE pid=? AND type!=? AND type!=? AND type!=? AND published=? AND (start=? OR start<?) AND (stop=? OR stop>?) ORDER BY sorting")
											   ->limit(1)
											   ->execute($objPages->id, 'root', 'error_403', 'error_404', 1, '', time(), '', time());
		...
		for ($i=(count($pages)-1); $i>0; $i--)
		{
			...
			(
				...
				'title' => specialchars($pages[$i]['title']),
				'link' => $pages[$i]['title']
			);
		}
