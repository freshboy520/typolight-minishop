...
class ModuleNewsReader extends ModuleNews
{
	...
	protected function compile()
	{
		...
		if ($objArchive->perPage > 0)
		{
			...
			$objTotal = $this->Database->prepare("SELECT COUNT(*) AS count FROM tl_news_comments WHERE pid=?" . (!BE_USER_LOGGED_IN ? " AND published=?" : ""))
		...
		$objCommentsStmt = $this->Database->prepare("SELECT * FROM tl_news_comments WHERE pid=?" . (!BE_USER_LOGGED_IN ? " AND published=?" : "") . " ORDER BY date" . (($objArchive->sortOrder == 'descending') ? " DESC" : ""));
		...
		$objComments = $objCommentsStmt->execute($objArticle->id, 1);
