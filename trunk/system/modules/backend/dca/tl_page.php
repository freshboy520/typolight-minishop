...
(
	...
	(
		...
		(
			array('tl_page', 'updateXmlSitemap')
		),
	...
	(
		...
		'root'                        => 'title,alias;type,dns,language,fallback;pageTitle,adminEmail;includeLayout;includeCache;includeChmod;createSitemap;published,start,stop',
	...
	(
		...
		'createSitemap'               => 'sitemapName,sitemapBase'
	),
	...
	(
		...
		(
			...
			'exclude'                 => true,
			'inputType'               => 'text',
			'eval'                    => array('rgxp'=>'url', 'doNotCopy'=>true, 'spaceToUnderscore'=>true, 'maxlength'=>64),
			'save_callback' => array
			(
		...
		'sitemapBase' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_page']['sitemapBase'],
			'default'                 => $this->Environment->base,
			'exclude'                 => true,
			'inputType'               => 'text',
			'eval'                    => array('mandatory'=>true, 'trailingSlash'=>true, 'rgxp'=>'url', 'decodeEntities'=>true, 'maxlength'=>255)
		),
		'hide' => array
		(
...
class tl_page extends Backend
{
	...
	public function checkPermission()
	{
		...
		if ($this->Input->get('act') == 'editAll' || $this->Input->get('act') == 'deleteAll')
		{
			...
			if (is_array($session['CURRENT']['IDS']))
			{
				...
				{
					$objPage = $this->Database->prepare("SELECT pid, includeChmod, chmod, cuser, cgroup FROM tl_page WHERE id=?")
											  ->limit(1)
											  ->execute($id);
	...
	public function pastePage(DataContainer $dc, $row, $table, $cr, $arrClipboard=false)
	{
		...
		if ($row['id'] > 0)
		{
			$return = $disablePA ? $this->generateImage('pasteafter_.gif').' ' : '<a href="'.$this->addToUrl('act='.$arrClipboard['mode'].'&amp;mode=1&amp;pid='.$row['id'].'&amp;id='.$arrClipboard['id']).'" title="'.specialchars(sprintf($GLOBALS['TL_LANG'][$table]['pasteafter'][1], $row['id'])).'" onclick="Backend.getScrollOffset();">'.$imagePasteAfter.'</a> ';
		}

		return $return.($disablePI ? $this->generateImage('pasteinto_.gif').' ' : '<a href="'.$this->addToUrl('act='.$arrClipboard['mode'].'&amp;mode=2&amp;pid='.$row['id'].'&amp;id='.$arrClipboard['id']).'" title="'.specialchars(sprintf($GLOBALS['TL_LANG'][$table]['pasteinto'][1], $row['id'])).'" onclick="Backend.getScrollOffset();">'.$imagePasteInto.'</a> ');
	}
	 ...
	 * @param object
	 */
	public function updateXmlSitemap(DataContainer $dc)
	{
		// Unpublish the page that is being deleted so it will not be included in the sitemap
		if ($this->Input->get('act') == 'delete')
		{
			$this->Database->prepare("UPDATE tl_page SET published=? WHERE id=?")
						   ->execute('', $dc->id);
		}
