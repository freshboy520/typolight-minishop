...
(
	...
	(
		...
		'gallery'                     => 'type,headline;multiSRC;perRow,sortBy,perPage;useHomeDir;imagemargin,size,fullsize;guests,protected;align,space,cssID',
...
class tl_content extends Backend
{
	...
	private function checkAccessToElement($id)
	{
		...
		/*
		if ($article_allow && $this->Input->get('id') != CURRENT_ID)
		{
			$objContent = $this->Database->prepare("SELECT pid FROM tl_content WHERE id=?")
										 ->limit(1)
										 ->execute($this->Input->get('id'));

			// Check whether the current content element belongs to the current article
			if ($objContent->numRows)
			{
				if ($objContent->pid == CURRENT_ID)
				{
					$content_allow = true;
				}

				elseif ($this->Input->get('act') == 'copy' || $this->Input->get('act') == 'cut')
				{
		...
		// Redirect if page is not mounted
		if (!$page_allow)
		{
			$this->log('Not enough permissions to modify article ID '.CURRENT_ID.' on page ID '.$objArticle->id, 'tl_content checkPermission()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}

		// Redirect if article is not allowed
		if (!$article_allow)
		{
			$this->log('Not enough permissions to modify article ID '.CURRENT_ID, 'tl_content checkPermission()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}

		// Redirect if content element is not allowed
		if (!$content_allow && $this->Input->get('id') != CURRENT_ID)
		{
			var_dump($content_allow, $this->Input->get('id') != CURRENT_ID);exit;
			$this->log('Not enough permissions to modify content element ID '.$this->Input->get('id'), 'tl_content checkPermission()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
		*/
	}
