		...
		if ($this->Input->post('FORM_SUBMIT') == 'tl_upload')
		{
			...
			foreach ($_FILES as $file)
			{
				...
				if (($arrImageSize = @getimagesize($file['tmp_name'])) != false)
				{
					...
					{
						$_SESSION['TL_ERROR'][] = sprintf($GLOBALS['TL_LANG']['ERR']['filewidth'], $file['name'], $GLOBALS['TL_CONFIG']['imageWidth']);
						$this->log('File "'.$file['name'].'" exceeds the maximum image width of '.$GLOBALS['TL_CONFIG']['imageWidth'].' pixels', 'DC_Folder move()', TL_ERROR);
					...
					{
						$_SESSION['TL_ERROR'][] = sprintf($GLOBALS['TL_LANG']['ERR']['fileheight'], $file['name'], $GLOBALS['TL_CONFIG']['imageHeight']);
						$this->log('File "'.$file['name'].'" exceeds the maximum image height of '.$GLOBALS['TL_CONFIG']['imageHeight'].' pixels', 'DC_Folder move()', TL_ERROR);
				...
				if ($this->Files->move_uploaded_file($file['tmp_name'], $this->Input->get('pid', DECODE_ENTITIES) . '/' . $file['name']))
				{
					$this->Files->chmod($this->Input->get('pid', DECODE_ENTITIES) . '/' . $file['name'], 0644);
...
<div class="tl_submit_container">
<input type="submit" name="upload" id="upload" class="tl_submit" alt="upload files" value="'.specialchars($GLOBALS['TL_LANG'][$this->strTable]['upload']).'" />
</div>
...
<a href="'.$this->getReferer(ENCODE_AMPERSANDS).'" class="header_back" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['backBT']).'" accesskey="b" onclick="Backend.getScrollOffset();">'.$GLOBALS['TL_LANG']['MSC']['backBT'].'</a>
	...
	public function source()
	{
		...
		{
			// Save file
			if ($md5 != md5($this->Input->postRaw('source')))
			{
...
<div class="tl_tbox">
  ...
  <textarea name="source" id="ctrl_source" class="tl_textarea monospace" rows="12" cols="80" style="height:400px;" onfocus="Backend.getScrollOffset();">' . $strContent . '</textarea>' . (($GLOBALS['TL_CONFIG']['showHelp'] && strlen($GLOBALS['TL_LANG']['tl_files']['editor'][1])) ? '
...
<input type="submit" name="saveNclose" id="saveNclose" class="tl_submit" alt="save all changes and return" accesskey="c" value="'.specialchars($GLOBALS['TL_LANG']['MSC']['saveNclose']).'" />
	...
	public function protect()
	{
		...
		{
			$this->log('Folder "' . $this->intId . '" contains a .htaccess file already', 'DC_Folder protect()', TL_ERROR);
			$this->redirect('typolight/main.php?act=error');
		}
	...
	private function generateTree($path, $intMargin, $mount=false, $blnProtected=false)
	{
		...
		for ($h=0; $h<count($files); $h++)
		{
			...
			if ($objFile->isGdImage && $objFile->height > 0)
			{
				...
				$popupHeight = ($objFile->height + 234);
				...
				if ($GLOBALS['TL_CONFIG']['thumbnails'])
				{
					...
					$thumbnail = '<br /><a href="typolight/preview.php?src='.$currentEncoded.'" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['view']).'" onclick="this.blur(); Backend.openWindow(this, '.$popupWidth.', '.$popupHeight.'); return false;" ><img src="' . $this->getImage($currentEncoded, $_width, $_height) . '" alt="" style="margin:0px 0px 2px 23px;" /></a>';
				}
			}

			$_buttons = '&nbsp;';
			$return .= '<a href="typolight/preview.php?src='.$currentEncoded.'" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['view']).'" onclick="this.blur(); Backend.openWindow(this, '.$popupWidth.', '.$popupHeight.'); return false;" >' . $this->generateImage($objFile->icon).' '.utf8_convert_encoding(basename($currentFile), $GLOBALS['TL_CONFIG']['characterSet']).'</a>'.$thumbnail.'</div> <div class="tl_right">';
