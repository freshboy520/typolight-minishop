...
class BackendUser extends User
{
	...
	public function __destruct()
	{
		...
		// File manager
		if ($this->Environment->script == 'typolight/files.php' && $session['referer']['current'] != $this->Environment->requestUri && !$this->Input->get('act') && !$this->Input->get('token'))
		{
	...
	/**
	 * Extend the parent authenticate function and redirect a back end user to typolight/index.php if there is no cookie
	 */
	public function authenticate()
	{
		if (!parent::authenticate() && $this->Environment->script != 'typolight/index.php')
		{
			...
			{
				echo '<script type="text/javascript">location.replace("' . $this->strLoginScript . '")</script>';
				exit;
			}

			$this->redirect('typolight/index.php');
		}
	}
	...
	protected function setUserFromDb()
	{
		...
		{
			$objGroup = $this->Database->prepare("SELECT * FROM tl_user_group WHERE id=? AND disable!=? AND (start=? OR start<?) AND (stop=? OR stop>?)")
									   ->limit(1)
