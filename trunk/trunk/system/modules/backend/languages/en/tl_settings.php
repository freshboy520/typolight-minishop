...
$GLOBALS['TL_LANG']['tl_settings']['extractTplVars']      = array('Extract template variables', 'Choose this option if you want to use normal template variables (e.g. <em>$language</em> instead of <em>$this->language</em>). If you are not using any custom templates you should disable this option to gain performance.');
...
$GLOBALS['TL_LANG']['tl_settings']['dynamicStopword']     = array('Dynamic stop words (search engine)', 'Stop words are words that are not indexed for searching. Dynamic stop words are words that occur very often and therefore are ignored. You can modify the threshold for dynamic stopwords here.');
$GLOBALS['TL_LANG']['tl_settings']['minWordLength']       = array('Minimum word length (search engine)', 'Please enter the minimum word length. Words with less characters will not be indexed for searching. Please note that a smaller number will increase the loading time of your website and the size of your database.');
