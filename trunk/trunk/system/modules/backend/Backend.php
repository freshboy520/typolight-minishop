...
abstract class Backend extends Controller
{
	 ...
	 * @return array
	 */
	protected function getSearchablePages($pid=0)
	{
		$objPages = $this->Database->prepare("SELECT * FROM tl_page WHERE pid=? AND (start=? OR start<?) AND (stop=? OR stop>?) ORDER BY sorting")
		...
		{
			// Add regular pages
			if ($objPages->type == 'regular')
			{
				// Searchable and not protected
				if (!$objPages->noSearch && !$objPages->protected)
				{
					// Published
					if ($objPages->published && (!$objPages->start || $objPages->start < time()) && (!$objPages->stop || $objPages->stop > time()))
					{
						$arrPages[] = $this->generateFrontendUrl($objPages->row());

						// Get teaser articles
						$objArticle = $this->Database->prepare("SELECT * FROM tl_article WHERE pid=? AND (start=? OR start<?) AND (stop=? OR stop>?) AND published=? AND showTeaser=? ORDER BY sorting")
													 ->execute($objPages->id, '', time(), '', time(), 1, 1);

						while ($objArticle->next())
						{
							$arrPages[] = $this->generateFrontendUrl($objPages->row(), '/articles/' . ((strlen($objArticle->alias) && !$GLOBALS['TL_CONFIG']['disableAlias']) ? $objArticle->alias : $objArticle->id));
						}
					}
				}
			}
			...
			// Get subpages
			if (!$objPages->protected && ($arrSubpages = $this->getSearchablePages($objPages->id)) != false)
			{
