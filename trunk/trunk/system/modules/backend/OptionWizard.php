...
class OptionWizard extends Widget
{
  ...
  <thead>
    <tr>
      <th>'.$GLOBALS['TL_LANG']['tl_form_field']['opValue'].'</th>
      <th>'.$GLOBALS['TL_LANG']['tl_form_field']['opLabel'].'</th>
      <th></th>
      <th></th>
    </tr>
  </thead>
  <tbody>';
    ...
    <tr>
      ...
      <td><input type="checkbox" name="'.$this->strId.'['.$i.'][default]" id="'.$this->strId.'_default_'.$i.'" class="fw_checkbox" value="1"'.($this->varValue[$i]['default'] ? ' checked="checked"' : '').' /> <label for="'.$this->strId.'_default_'.$i.'">'.$GLOBALS['TL_LANG']['tl_form_field']['opDefault'].'</label></td>
      <td><input type="checkbox" name="'.$this->strId.'['.$i.'][group]" id="'.$this->strId.'_group_'.$i.'" class="fw_checkbox" value="1"'.($this->varValue[$i]['group'] ? ' checked="checked"' : '').' /> <label for="'.$this->strId.'_group_'.$i.'">'.$GLOBALS['TL_LANG']['tl_form_field']['opGroup'].'</label></td>';
      ...
      <td style="white-space:nowrap; padding-left:3px;">';
			...
			{
				$return .= '<a href="'.$this->addToUrl('&amp;command='.$button.'&amp;cid='.$i.'&amp;id='.$this->currentRecord).'" title="'.specialchars($GLOBALS['TL_LANG'][$this->strTable][$button][0]).'" onclick="Backend.optionsWizard(this, \''.$button.'\', \'ctrl_'.$this->strId.'\'); return false;">'.$this->generateImage($button.'.gif', $GLOBALS['TL_LANG'][$this->strTable][$button][0]).'</a> ';
			}
