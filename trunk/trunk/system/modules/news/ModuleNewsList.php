...
class ModuleNewsList extends ModuleNews
{
	...
	protected function compile()
	{
		...
		if ($this->perPage > 0)
		{
			...
			$objTotalStmt = $this->Database->prepare("SELECT id AS count FROM tl_news WHERE pid IN(" . implode(',', $this->news_archives) . ")" . (!BE_USER_LOGGED_IN ? " AND published=?" : "") . " ORDER BY date DESC");
		...
		$objArticles = $objArticlesStmt->execute(1);
