...
abstract class Module extends Frontend
{
	...
	protected function renderNavigation($pid, $level=1)
	{
		...
		$objSubpages = $this->Database->prepare("SELECT * FROM tl_page WHERE pid=? AND type!=? AND type!=? AND type!=? AND (start=? OR start<?) AND (stop=? OR stop>?)" . ((!$this instanceof ModuleSitemap || !$this->showHidden) ? ' AND hide!=?' : '') . ((!$this instanceof ModuleSitemap && FE_USER_LOGGED_IN && !BE_USER_LOGGED_IN) ? ' AND guests!=?' : '') . (!BE_USER_LOGGED_IN ? ' AND published=?' : '') . " ORDER BY sorting")
		...
		while($objSubpages->next())
		{
			...
			if (!strlen($objSubpages->protected) || (!is_array($_groups) && FE_USER_LOGGED_IN) || BE_USER_LOGGED_IN || (is_array($_groups) && count(array_intersect($groups, $_groups))) || $this->showProtected)
			{
				...
				// Active page
				if ($objPage->id == $objSubpages->id && !$this instanceof ModuleSitemap)
				{
