...
class ModuleCustomnav extends Module
{
	...
	protected function compile()
	{
		...
		{
			$objPages = $this->Database->prepare("SELECT * FROM tl_page WHERE id=? AND type!=? AND type!=? AND type!=? AND (start=? OR start<?) AND (stop=? OR stop>?)" . ((FE_USER_LOGGED_IN && !BE_USER_LOGGED_IN) ? ' AND guests!=?' : '') . (!BE_USER_LOGGED_IN ? ' AND published=?' : ''))
									   ->limit(1)
