...
class ContentDownload extends ContentElement
{
	...
	protected function compile()
	{
		...
		{
			$this->linkTitle = basename($objFile->basename);
		}
		...
		$this->Template->href = $this->Environment->request . (($GLOBALS['TL_CONFIG']['disableAlias'] && $this->Input->get('id')) ? '&amp;' : '?') . 'file=' . $this->urlEncode($this->singleSRC);
	}
}
