...
class ModuleQuicklink extends Module
{
	...
	protected function compile()
	{
		...
		{
			$objPage = $this->Database->prepare("SELECT id, title, alias FROM tl_page WHERE id=? AND type!=? AND type!=? AND type!=? AND (start=? OR start<?) AND (stop=? OR stop>?)" . ((FE_USER_LOGGED_IN && !BE_USER_LOGGED_IN) ? ' AND guests!=?' : '') . (!BE_USER_LOGGED_IN ? ' AND published=?' : ''))
									  ->limit(1)
									  ->execute($intId, 'root', 'error_403', 'error_404', '', time(), '', time(), 1, 1);
