...
class ModuleSearch extends Module
{
	...
	protected function compile()
	{
		...
		$objFormTemplate->keyword = specialchars($this->Input->get('keywords'));
		...
		if (strlen(trim($this->Input->get('keywords'))) && $this->Input->get('keywords') != '*')
		{
			global $objPage;
			...
			$strChecksum = md5($this->Input->get('keywords').$this->Input->get('query_type').$objPage->rootId);
			...
			if (is_null($arrResult))
			{
				...
				$objSearch = $this->Search->searchFor($this->Input->get('keywords'), ($this->Input->get('query_type') == 'or'), $arrPages);
			...
			if ($count < 1)
			{
				$this->Template->header = sprintf($GLOBALS['TL_LANG']['MSC']['sEmpty'], $this->Input->get('keywords'));
			...
			$this->Template->header = vsprintf($GLOBALS['TL_LANG']['MSC']['sResults'], array($from, $to, $count, $this->Input->get('keywords')));
			$this->Template->duration = substr($query_endtime-$query_starttime, 0, 6) . ' ' . $GLOBALS['TL_LANG']['MSC']['seconds'];
		}
	}
}
