...
class FrontendUser extends User
{
	...
	public function __destruct()
	{
		...
		if (!isset($_GET['pdf']) && $session['referer']['current'] != $this->Environment->requestUri)
		{
	...
	protected function setUserFromDb()
	{
		...
		$objGroups = $this->Database->prepare("SELECT id FROM tl_member_group WHERE disable!=? AND (start=? OR start<?) AND (stop=? OR stop>?)")
									->execute(1, '', time(), '', time());

		$this->groups = array_intersect($this->groups, $objGroups->fetchEach('id'));

		// Get group login page
		$objGroup = $this->Database->prepare("SELECT * FROM tl_member_group WHERE id=? AND disable!=? AND (start=? OR start<?) AND (stop=? OR stop>?)")
								   ->limit(1)
								   ->execute($this->groups[0], 1, '', time(), '', time());
