...
class ModuleArticleList extends Module
{
	...
	protected function compile()
	{
		...
		$objArticles = $this->Database->prepare("SELECT id, title, inColumn, cssID FROM tl_article WHERE pid=? AND inColumn=? AND (start=? OR start<?) AND (stop=? OR stop>?)" . (!BE_USER_LOGGED_IN ? ' AND published=?' : '') . " ORDER BY sorting")
									  ->execute($id, $this->inColumn, '', time(), '', time(), 1);
