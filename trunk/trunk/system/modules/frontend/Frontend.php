...
abstract class Frontend extends Controller
{
	...
	protected function getPageIdFromUrl()
	{
		...
		$strRequest = preg_replace('/' . preg_quote(URL_SUFFIX, '/') . '$/i', '', $strRequest);
	...
	protected function getRootIdFromUrl()
	{
		...
		$objRootPage = $this->Database->prepare("SELECT id FROM tl_page WHERE type=? AND (dns=? OR dns=?) AND language=? AND (start=? OR start<?) AND (stop=? OR stop>?)" . (!BE_USER_LOGGED_IN ? ' AND published=?' : ''))
									  ->limit(1)
									  ->execute('root', $host, 'www.'.$host, $accept_language[0], '', time(), '', time(), 1);

		if ($objRootPage->numRows < 1)
		{
			// Current host matches a record (look for fallback language)
			$objRootPage = $this->Database->prepare("SELECT id FROM tl_page WHERE type=? AND (dns=? OR dns=?) AND fallback=? AND (start=? OR start<?) AND (stop=? OR stop>?)" . (!BE_USER_LOGGED_IN ? ' AND published=?' : ''))
										  ->limit(1)
										  ->execute('root', $host, 'www.'.$host, 1, '', time(), '', time(), 1);
		}

		if ($objRootPage->numRows < 1)
		{
			// Current user language matches a record (and DNS is empty)
			$objRootPage = $this->Database->prepare("SELECT id FROM tl_page WHERE type=? AND dns=? AND language=? AND (start=? OR start<?) AND (stop=? OR stop>?)" . (!BE_USER_LOGGED_IN ? ' AND published=?' : ''))
										  ->limit(1)
										  ->execute('root', '', $accept_language[0], '', time(), '', time(), 1);
		}

		if ($objRootPage->numRows < 1)
		{
			// Current fallback language matches a record (and DNS is empty)
			$objRootPage = $this->Database->prepare("SELECT id FROM tl_page WHERE type=? AND dns=? AND fallback=? AND (start=? OR start<?) AND (stop=? OR stop>?)" . (!BE_USER_LOGGED_IN ? ' AND published=?' : ''))
										  ->limit(1)
	...
	protected function addToUrl($strRequest)
	{
		...
		return ($GLOBALS['TL_CONFIG']['rewriteURL'] ? '' : 'index.php/') . $this->getPageIdFromUrl() . $strParams . URL_SUFFIX;
	}
	 ...
	 * @return string
	 */
	protected function createDownloadToken($strFile)
	{
		$strToken = md5(uniqid('', true));

		if (!in_array($strFile, $_SESSION['downloadFiles']))
		{
			$_SESSION['downloadFiles'][] = $strFile;
		}

		if (!in_array($strToken, $_SESSION['downloadToken']))
		{
			$_SESSION['downloadToken'][] = $strToken;
		}
	...
	{
		if (!file_exists(TL_ROOT . '/' . $strPath . '/meta.txt') || in_array($strPath, $this->arrProcessed))
		{
			return;
		}

		$strBuffer = file_get_contents(TL_ROOT . '/' . $strPath . '/meta.txt');
