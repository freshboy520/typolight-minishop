...
class ContentComments extends ContentElement
{
	...
	protected function compile()
	{
		...
		if ($this->com_perPage > 0)
		{
			...
			$objTotal = $this->Database->prepare("SELECT COUNT(*) AS count FROM tl_comments WHERE pid=?" . (!BE_USER_LOGGED_IN ? " AND published=?" : ""))
									   ->execute($this->id, 1);
		...
		$objCommentsStmt = $this->Database->prepare("SELECT * FROM tl_comments WHERE pid=?" . (!BE_USER_LOGGED_IN ? " AND published=?" : "") . " ORDER BY date" . (($this->com_order == 'descending') ? " DESC" : ""));
		...
		$objComments = $objCommentsStmt->execute($this->id, 1);
